...
describe("agregar e o veredito", () => {
  const classes = (r: number, s: number, c: number): Classe[] => [
  ...
  it("conta cada classe e o total", () => {
    expect(agregar(classes(2, 3, 4))).toEqual({ real: 2, raso: 3, cadaver: 4, total: 9 });
  });

  it("sem descarte não há o que julgar", () => {
    expect(vereditoDoTeto(agregar([]), 0.3, 0.55)).toMatch(/não há o que julgar/);
  });
  ...
  it("REAL = 0 diz que as mesas estão CORRETAMENTE paradas", () => {
    ...
    const t = vereditoDoTeto(agregar(classes(0, 5, 2)), 0.3, 0.55);
    expect(t).toMatch(/corretamente paradas/);
    expect(t).toMatch(/não paga neste custo/);
    expect(t).not.toMatch(/há número para discutir/);
  });

  it("REAL > 0 acusa o teto, e NÃO autoriza mexer nele", () => {
    const t = vereditoDoTeto(agregar(classes(3, 5, 2)), 0.3, 0.55);
  ...
  it("o veredito carrega os DOIS números da janela", () => {
    const t = vereditoDoTeto(agregar(classes(1, 0, 0)), 0.3, 0.55);
