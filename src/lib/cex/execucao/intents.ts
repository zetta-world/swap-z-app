...
/**
 * ⚠️ A124 (round 6): `ordensConhecidas` e `tradesNoLivro` SEM escopo foram
 * REMOVIDOS daqui de propósito — eles consultavam por `exchange_id` (+
 * símbolo) ignorando a CONTA, e fill/ordem de outra conta absolvia atividade
 * externa desta. Os sucessores com escopo vivem em
 * `src/lib/cex/execucao/escopo-de-conta.ts`. A remoção é a guarda: qualquer
 * caller antigo quebra no `tsc` em vez de voltar a consultar exchange-wide.
 */
