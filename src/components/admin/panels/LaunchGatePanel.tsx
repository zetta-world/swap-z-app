  ...
  return (
    ...
    >
      ...
      {data && (
        <div>
          <div style={{
            fontSize: 12, padding: "7px 9px", borderRadius: 3, marginBottom: 10, lineHeight: 1.5,
            ...
            return (
              <div key={d.source} style={{ marginBottom: 6, borderLeft: `2px solid ${d.passed ? "var(--adm-green)" : "var(--adm-border)"}`, paddingLeft: 7 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8, cursor: "pointer" }}
                  ...
                  <span style={{ fontSize: 13, color: "var(--adm-ink-2)", flex: 1 }}>{d.name}</span>
                  <span style={{ fontSize: 12, color: d.passed ? "var(--adm-green)" : "var(--adm-ink-3)" }}>
                    {ok}/5
                  </span>
                  <span style={{ fontSize: 12, color: "var(--adm-ink-3)", fontVariantNumeric: "tabular-nums" }}>
                    {usd(d.usdt)}
                  </span>
                ...
                {d.criteria.map((c) => {
                  ...
                  return (
                    <div key={c.id} style={{ marginTop: 3 }}>
                      ...
                      {isOpen && (
                        <div style={{ fontSize: 10, color: "var(--adm-ink-4)", paddingLeft: 15, fontStyle: "italic", lineHeight: 1.5 }}>
                          {c.why}
                        </div>
                      )}
                    </div>
          ...
          <div style={{ fontSize: 10, color: "var(--adm-ink-4)", marginTop: 10, fontStyle: "italic", lineHeight: 1.6 }}>
