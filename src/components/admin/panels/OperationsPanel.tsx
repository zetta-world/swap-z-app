  ...
  return (
    <TerminalPanel id="live-ops" title="LIVE OPS" subtitle="open positions · run feed" icon="⊠" source="supabase/autopilot_positions+runs">
      <div style={{ display: "flex", gap: 6, marginBottom: 12, alignItems: "center" }}>
        ...
        <div style={{ flex: 1 }} />
        {data && (
          <span style={{ fontSize: 12, color: "var(--adm-ink-3)" }}>
      ...
      {data && tab === "positions" && (
        data.positions.length === 0 ? (
          <div style={{ color: "var(--adm-ink-3)", fontSize: 13 }}>No open autopilot positions.</div>
        ) : (
          <table className="adm-table">
            ...
            <tbody>
              {data.positions.map((p, i) => (
                <tr key={i}>
                  <td style={{ color: "var(--adm-ink)" }}>{p.base}<span style={{ color: "var(--adm-ink-4)", fontSize: 11 }}> {p.exchange_id}</span></td>
      ...
      {data && tab === "runs" && (
        <div className="adm-scroll" style={{ maxHeight: 280 }}>
          {data.runs.length === 0 ? (
            <div style={{ color: "var(--adm-ink-3)", fontSize: 13 }}>No autopilot runs yet.</div>
          ) : data.runs.map((r, i) => (
