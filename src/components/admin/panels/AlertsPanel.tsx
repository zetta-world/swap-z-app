      ...
      {data && (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 13, color: data.configured ? "var(--adm-green)" : "var(--adm-gold)", flex: 1 }}>
          ...
          {testMsg && <div style={{ fontSize: 12, color: "var(--adm-cyan)", marginBottom: 8 }}>{testMsg}</div>}

          <div className="adm-category">Recent alerts</div>
          <div className="adm-scroll" style={{ maxHeight: 240 }}>
            {data.recent.length === 0 ? (
              <div style={{ color: "var(--adm-ink-3)", fontSize: 13 }}>No alerts fired yet. ✓</div>
            ) : data.recent.map((a, i) => (
