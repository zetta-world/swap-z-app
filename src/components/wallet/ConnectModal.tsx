...
/**
 * Wallet picker modal. Solana-first: Phantom/Solflare appear at the top.
 *
 * On mobile browsers, Phantom can't inject into third-party browsers (only
 * into its own in-app browser). We detect that case and show a deep-link
 * that opens the current page inside Phantom's browser — where it CAN
 * inject — instead of a misleading "Install" prompt.
 */
export default function ConnectModal({
  ...
  const phantomBrowseUrl = typeof window !== "undefined"
    ? `https://phantom.app/ul/v1/browse/${encodeURIComponent(window.location.href)}?ref=${encodeURIComponent(window.location.origin)}`
  ...
  const onConnect = async (c: Connector) => {
    ...
    try {
      try {
        ...
        // wagmi v2 throws "Connector already connected" on stale state.
        // Disconnect + retry gives the user one-click recovery.
            ...
            <div className="rounded-[19px] glass-strong flex flex-col flex-1 min-h-0">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
                ...
                {solana.wallets.length > 0 && (
                  <>
                    <div className="px-1 py-1 font-mono text-[9px] tracking-widest uppercase flex items-center gap-2" style={{ color: "#14F195" }}>
                      <span>{t("topbar.sectionSolana")}</span>
                    ...
                    {solana.wallets.map((w, i) => {
                      ...
                      const isPhantom = w.adapter.name.toLowerCase() === "phantom";

                      const icon = w.adapter.icon
                        ? <img src={w.adapter.icon} alt="" loading="lazy" className="w-9 h-9 rounded-lg object-cover flex-shrink-0" /> // eslint-disable-line @next/next/no-img-element
                        : <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 font-display font-extrabold text-xs" style={{ background: "#14F19522", color: "#14F195", border: "1px solid #14F19555" }}>{w.adapter.name.slice(0, 2).toUpperCase()}</div>;

                      // Mobile + Phantom not injected → deep-link into Phantom's browser
                      if (isPhantom && notDetected && isMobile) {
                        return (
                          <motion.a
                            key={id}
                            ...
                            className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl border transition-all text-left group border-[#14F195]/30 bg-[#14F195]/[0.04] hover:bg-[#14F195]/[0.08]"
                          >
                            {icon}
                            <div className="flex-1 min-w-0">
                              <div className="font-display font-bold text-sm text-ink truncate">{w.adapter.name}</div>
                              <div className="font-mono text-[10px] uppercase tracking-wider truncate" style={{ color: "#14F195" }}>
                                {t("topbar.solOpenInApp")}
                              </div>
                            </div>
                            <span className="font-mono text-[10px] tracking-widest uppercase" style={{ color: "#14F195" }}>
                              {t("topbar.openArrow")}
                            </span>
                          </motion.a>
                        );
                ...
                {/* ── EVM ─────────────────────────────────────────────── */}
                {sorted.length === 0 && solana.wallets.length === 0 && (
                ...
                {sorted.map((c, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-display font-bold text-sm text-ink truncate">{meta.name}</div>
                        <div className="font-mono text-[10px] text-ink-3 uppercase tracking-wider truncate">{meta.subtitle}</div>
                      </div>
                      {isPendingThis ? (
...
function getWalletMeta(c: Connector): WalletMeta {
  ...
  if (id.includes("walletconnect") || c.type === "walletConnect") return { name: "WalletConnect", subtitle: "Scan QR with mobile wallet", color: "#3B99FC", iconUrl };
  if (id === "injected" || c.type === "injected")           return { name: name || "Browser Wallet", subtitle: "Detected extension", color: "#00E8FF", iconUrl };
...
function WalletIcon({ meta }: { meta: WalletMeta }) {
  if (meta.iconUrl) {
    ...
    return <img src={meta.iconUrl} alt="" loading="lazy" className="w-9 h-9 rounded-lg object-cover flex-shrink-0" />;
  }
  return (
    <div
...
function dedupeConnectors(connectors: readonly Connector[]): Connector[] {
  ...
  for (const c of connectors) {
    ...
    const key = isMetaMask ? "metamask" : isCoinbase ? "coinbase" : isWC ? "walletconnect" : name;
    const existing = byKey.get(key);
    if (!existing) { byKey.set(key, c); continue; }
    if (isMetaMask) {
      if (c.id === "metaMask" && existing.id !== "metaMask") byKey.set(key, c);
      continue;
    }
