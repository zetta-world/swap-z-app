...
vi.mock("@/lib/cex/server", async (importOriginal) => {
  ...
  return {
    ...
    instanciarExchange: async () => ({
      ...
      fetchOrder: (...a: unknown[]) => estadoDaVenue.fetchOrderImpl(...a),
      fetchMyTrades: (...a: unknown[]) => estadoDaVenue.fetchMyTradesImpl(...a),
      fetchClosedOrders: (...a: unknown[]) => estadoDaVenue.fetchClosedOrdersImpl(...a),
    }),
  };
});
...
describe("A80/A102 — recovery por intentId", () => {
  ...
  it("⚠️⚠️ intent de PILOTO (com session_id) → recusa use_a_sessao, sem tocar na venue", async () => {
    const intent = await plantarIntent({ session_id: "sess-1" });
    venueComFill();
    const res = await POST(reqDe({ intentId: intent.id, ...CREDS }));
