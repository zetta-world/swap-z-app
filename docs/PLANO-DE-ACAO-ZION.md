...
| Z4 | **Contexto macro** — dominância BTC/ETH + mcap total (CoinGecko), supply de stablecoins (DefiLlama), DXY+S&P (Yahoo, best-effort) | `src/lib/api/macro.ts`; ligado em `buildPairData` + backtester | 🟢 |
...
| Z8 | **Ampliar `confidenceScore`** (6→9 dims: +ciclo 1Y, +semanal, +divergência) e centralizar pesos em `SCORE_WEIGHTS` p/ recalibração | `computeConfidenceScore` | 🟡 |
...
| E4 | Smart Money / Whale no ZION | 🟢 CONCLUÍDO (2026-06-27) — `analyzeSmartMoney` (server, mesmos thresholds do ProSmartMoney) injeta verdict ACCUMULATING/DISTRIBUTING no prompt, do pool do token analisado. (Grafo persistente de carteiras lucrativas = futuro.) | 🟢 |
